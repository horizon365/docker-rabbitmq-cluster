...
def get_field_verbose_name(field):
    ...
    if isinstance(field, models.fields.reverse_related.ForeignObjectRel):
        ...
        verbose_dict['verbose_name'] = field.model._meta.verbose_name
        verbose_dict['related name of :attr:'] = (f"~{field.remote_field.model.__module__}"
            f".{field.remote_field.model.__name__}.{field.remote_field.name}")
        ...
        verbose_dict ={
            "specified in :attr:": f"{field.model.__module__}.{field.model.__name__}.{field.ct_field}"
        }
        (
    ...
    if isinstance(field, models.fields.related.RelatedField):
        ...
        verbose_dict = {
            'related name: :attr:': f"{to.__module__}.{to.__name__}.{related_name}"
        }
    return verbose_name